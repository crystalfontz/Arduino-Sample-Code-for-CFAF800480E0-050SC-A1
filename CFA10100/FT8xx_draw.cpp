...
uint16_t FT8xx_Point(uint16_t FWol,
  ...
  FWol=FT8xx_Cmd_Dat_0( FWol, POINT_SIZE(ball_size));
  // Indicate to draw a point (dot)
  FWol=FT8xx_Cmd_Dat_0( FWol, BEGIN(FTPOINTS));
  // Set the point center location
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(point_x,point_y));
  // End the point
...
uint16_t FT8xx_Line(uint16_t FWol,
  ...
  {
  //Set the line width
  FWol=FT8xx_Cmd_Dat_0( FWol, LINE_WIDTH(width*16));
  // Start a line
  FWol=FT8xx_Cmd_Dat_0( FWol, BEGIN(LINES));
  // Set the first point
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x0*16,y0*16));
  // Set the second point
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x1*16,y1*16));
  // End the line
  FWol=FT8xx_Cmd_Dat_0( FWol, END());
  //Give the updated write pointer back to the caller
  return(FWol);
...
uint16_t FT8xx_Filled_Rectangle(uint16_t FWol,
  ...
  FWol=FT8xx_Cmd_Dat_0( FWol, LINE_WIDTH(16));
  // Start a rectangle
  FWol=FT8xx_Cmd_Dat_0( FWol, BEGIN(RECTS));
  // Set the first point
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x0*16,y0*16));
  // Set the second point
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x1*16,y1*16));
...
uint16_t FT8xx_Open_Rectangle(uint16_t FWol,
  ...
  // Top
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x0*16,y0*16));
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x1*16,y0*16));
  //Right
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x1*16,y0*16));
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x1*16,y1*16));
  //Bottom
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x1*16,y1*16));
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x0*16,y1*16));
  //Left
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x0*16,y1*16));
  FWol=FT8xx_Cmd_Dat_0( FWol, VERTEX2F(x0*16,y0*16));
  // End the line set
...
uint16_t FT8xx_Text(uint16_t FWol,
  ...
  //Send the CMD_TEXT command
  _FT8xx_send_32(CMD_TEXT);
  //Send the parameters of the CMD_TEXT command
  //First is 32-bit combination of Y & X
  _FT8xx_send_32((((uint32_t)y)<<16) | (x));
  //Second is a combinations of options and the font=32
  _FT8xx_send_32((((uint32_t)Options)<<16) | (Font));
...
//  while(FT8xx_REG_Read_16(REG_CMD_READ) != FWol);
  //========== START THE DISPLAY LIST ==========
  // Start the display list
  FWol=FT8xx_Cmd_Dat_0( FWol, (CMD_DLSTART));
  // Set the default clear color to black
  FWol=FT8xx_Cmd_Dat_0( FWol, Clear_Color);
  ...
  FWol=FT8xx_Cmd_Dat_0( FWol, Spinner_Color);
  ...
  FWol=FT8xx_Cmd_Dat_0( FWol, DISPLAY());
  // Make this list active
  FWol=FT8xx_Cmd_Dat_0( FWol, (CMD_SWAP));
  // Update the ring buffer pointer so the graphics processor starts
  // executing.
  // Note: this is a write to register space not RAM_CMD space.
...
uint16_t Calibrate_Touch(uint16_t FWol)
  ...
  FT8xx_REG_Write_16(REG_CMD_WRITE, FWol);
  //Optional / debug
  SerPrintFF(F("Waiting for touch cal . . ."));
...
uint16_t FT8xx_Load_PNG_to_RAM_G(uint16_t FWol,
  ...
  //We know that the FT8xx will uncompress the image as a R5G6B5 image
  ...
  if((RAM_G_SIZE-*(RAM_G_Address)) < RAM_G_Needed)
    {
#if (0<CHATTY)
    SerPrintFF(F("Image is %lu bytes long, but only %lu are available.\n"),
  ...
  while(0 != PNG_length)
    ...
    //See how much room is available in the RAM_CMD
    bytes_free=Get_Free_CMD_Space(FWol);

 SerPrintFF(F("PNG_length= %lu bytes_free = %lu "),PNG_length,bytes_free);
 ...
 SerPrintFF(F("bytes_this_block = %lu \n"),bytes_this_block);

    //Set the address in RAM_CMD for this block
    _FT8xx_Select_and_Address((uint32_t)RAM_CMD|(uint32_t)FWol,
                              MEM_WRITE);
  ...
  *RAM_G_Address=(*RAM_G_Address+0x07)&0xFFFFFFF8;
  ...
  if((*RAM_G_Address!=Read_Back_RAM_G_First_Available)||
    ...
    {
    SerPrintFF(F("Read-back Error.\n"));
    SerPrintFF(F("Calc RAM_G %lu Read Back RAM_G = %lu\n"),
                 *RAM_G_Address,Read_Back_RAM_G_First_Available);
    SerPrintFF(F("Calc Wide %lu Read Back Wide = %lu\n"),
                 *Image_Width,Read_Back_Width);
    SerPrintFF(F("Calc High %lu Read Back High = %lu\n"),
                 *Image_Height,Read_Back_Height);
    }
#endif // (5<CHATTY)
...
uint16_t FT8xx_Inflate_to_RAM_G(uint16_t FWol,
  ...
  //Write the CMD_INFLATE and parameters
 ...
 SerPrintFF(F("data_length= %lu bytes_free = %lu "),data_length,bytes_free);
...
void Load_File_To_RAM_G(uint32_t RAM_G_Address,
                        ...
                        uint32_t *RAM_G_Used)
  {
  //Keep track of the amount of RAM_G we have used. To start
  //with it is 0, which also indicates any kind of gross uSD
  //failure, like no file, no card, etc.
//  *RAM_G_Used=0;

//Need to refactor this
// no reason to loop through root. it is there or not
// return an error code?
// check RAM_G size? Would need to know RAM_G total
// for our chip
  ...
  if(0 == root_dir)
    {
    SerPrintFF(F("Load_File_To_RAM_G: Can't open \"root\".\n"));
    return;
    }
  File
    binary_file;
  ...
  while(1)
    {
    binary_file = root_dir.openNextFile();
    if (0 == binary_file)
      {
    ...
    //Skip directories / volumes name
    if(0 == binary_file.isDirectory())
      {
      //Does the name of this file matching our target File_Name?
      if(0 == strcmp(binary_file.name(),File_Name))
        {
        SerPrintFF(F("Found: %s Size: %lu\n"),binary_file.name(),binary_file.size());
        //If the size is 0, then this is going to be a short trip.
        if(0==binary_file.size())
          {
        ...
        //of the file vs remaining RAM_G space.
        uint32_t
          bytes_remaining;
        ...
        *RAM_G_Used=bytes_remaining;
        ...
        do
          ...
          _FT8xx_Select_and_Address(RAM_G_Address,MEM_WRITE);
...
//SerPrintFF(F("waiting outside of loop . . ."));
//Make sure the chip is caught up.
//while(FT8xx_REG_Read_16(REG_CMD_READ) != FWol);
//SerPrintFF(F(" done.\n"));
